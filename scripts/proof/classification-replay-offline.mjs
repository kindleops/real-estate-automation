...
async function runReplay() {
  console.log("Starting Offline Traffic Replay Audit...");
  
  const data = JSON.parse(fs.readFileSync("replay_data.json", "utf8")).slice(0, 100);

  console.log(`Auditing ${data.length} messages...\n`);
  ...
  for (const event of data) {
    ...
    const old_conf = parseFloat(event.classification_confidence ?? 0);
    ...
    if ((old_intent === 'unclear' || !old_intent) && new_intent !== 'unclear') {
      metrics.improved++;
    }
  ...
  console.log(`Reduction in Unclear: ${(((metrics.old_unclear - metrics.new_unclear) / metrics.old_unclear) * 100).toFixed(2)}%`);
  ...
  // Filter out nulls
  const unresolved = metrics.unresolved_unclear.filter(Boolean);
  unresolved.slice(0, 10).forEach(msg => console.log(`- "${msg}"`));
}
